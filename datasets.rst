...
The collision data passing the CMS trigger selections (L1 & HLT) is organized in primary datasets (PD). All open data primary datasets can be found with `this search <http://opendata.cern.ch/search?page=1&size=20&type=Dataset&subtype=Collision&experiment=CMS>`_.
...
One year of data taking is divided in several "eras" indicated as RunA, RunB, etc. According to the CMS data policy, 50% of data is published after the embargo period, completed with the full release within 10 years. The data are reprocessed several times, and it is the last complete reprocessing available at the time of the release which is made public.
